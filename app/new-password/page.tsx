...
function parseHashTokens() {
  // Supabase puts tokens in URL hash: #access_token=...&refresh_token=...&type=recovery
...
export default function NewPasswordPage() {
  ...
  // 1) When user opens the email link, set session from tokens in URL
  useEffect(() => {
    const run = async () => {
      const { access_token, refresh_token, type } = parseHashTokens();

      if (!access_token || !refresh_token) {
        setErrorMsg("Invalid or expired reset link. Please request again.");
        setReady(true);
        return;
      }

      // Optional check: type === "recovery"
      if (type && type !== "recovery") {
        // still try, but show warning if needed
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Optional: sign out after reset (clean)
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  >
                    {show ? (
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                        <path d="M3 3l18 18" stroke="#111827" strokeWidth="1.6" strokeLinecap="round" />
                        <path d="M10.6 10.6a2 2 0 0 0 2.8 2.8" stroke="#111827" strokeWidth="1.6" strokeLinecap="round" />
                        <path d="M6.7 6.7C4.2 8.6 2.7 12 2.7 12s2.7 6 9.3 6c1.8 0 3.3-.4 4.6-1" stroke="#111827" strokeWidth="1.6" strokeLinecap="round" />
                        <path d="M9.3 4.3c.8-.2 1.7-.3 2.7-.3 6.6 0 9.3 8 9.3 8s-.9 2-2.8 3.7" stroke="#111827" strokeWidth="1.6" strokeLinecap="round" />
                      </svg>
                    ) : (
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                        <path d="M2.7 12s2.7-8 9.3-8 9.3 8 9.3 8-2.7 8-9.3 8-9.3-8-9.3-8Z" stroke="#111827" strokeWidth="1.6" />
                        <path d="M12 15.2a3.2 3.2 0 1 0 0-6.4 3.2 3.2 0 0 0 0 6.4Z" stroke="#111827" strokeWidth="1.6" />
                      </svg>
                    )}
                  </button>
                </div>
              ...
              <div className="flex justify-center pt-2">
                <button
                  type="submit"
                  disabled={loading}
                  className="rounded-full bg-[#244a9b] px-12 py-2.5 text-sm font-semibold text-white hover:opacity-95 disabled:opacity-60"
                >
